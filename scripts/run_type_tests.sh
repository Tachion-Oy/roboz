...
VALID_DIR="$ROOT_DIR/tests/primitives/type_tests/cases/valid"
EXPECTED_FAILURE_DIR="$ROOT_DIR/tests/primitives/type_tests/cases/expected_failures"
CONFIG="$ROOT_DIR/pyrightconfig.type-tests.json"
VERBOSE="${TYPE_TESTS_VERBOSE:-0}"
if [[ -x "$ROOT_DIR/.venv/bin/pyright" ]]; then
  PYRIGHT=("$ROOT_DIR/.venv/bin/pyright")
else
...
valid_output=""
if ! valid_output="$("${PYRIGHT[@]}" --project "$CONFIG" "$VALID_DIR" 2>&1)"; then
...
for file in "${expected_failure_files[@]}"; do
  name="$(basename "$file")"
  output=""
  if output="$("${PYRIGHT[@]}" --project "$CONFIG" "$file" 2>&1)"; then
