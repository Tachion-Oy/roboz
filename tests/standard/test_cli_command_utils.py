...
from roboz.standard.skills.cli_commands.tools.run_file_command import _validate_input
from roboz.standard.skills.cli_commands.tools.resolve import (
    _guard_items,
...
from roboz.standard.skills.cli_commands.utilities.cmd_spec import ArgPattern, CmdSpec
from roboz.standard.skills.cli_commands.utilities.formatting import (
...
def test_format_cli_constraints_reflects_permission_rules(
    tmp_path: Path,
) -> None:
    """format_cli_constraints reflects configured allow and deny rules."""
...
def test_help_command_returns_help_output(tmp_path: Path) -> None:
    ...
    # Run the chain: run_file_command -> guard_operation -> execute_file_command
...
def test_validate_input_rejects_mv_clobber_flags(flag: str) -> None:
    input = RunFileCommands(
        chain="pipe",
        file_commands=[
            RunFileCommand(command="mv", argv=[flag, "src.txt", "dst.txt"])
        ],
    )
    err = _validate_input(input, FILE_COMMANDS)
    assert err is not None
