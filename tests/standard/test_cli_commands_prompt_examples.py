"""Regression tests for `cli_commands` prompt examples.
...
from roboz.models import Message, Role, Stop
...
from roboz.standard.skills.cli_commands.prompts import INSTRUCTIONS
from roboz.standard.skills.cli_commands.runtime import runner as command_runner
    ...
    [
        ...
        (
            RunFileCommands(
                ...
                file_commands=[
                    RunFileCommand(
                        command="rg", argv=["-n", "roboz|site-packages", "."]
                    )
                ],
            ),
            [["rg", "-n", "roboz|site-packages", "{BASE}"]],
        ),
        (
            RunFileCommands(
                chain="pipe",
        ...
        (
                ...
                file_commands=[
                    RunFileCommand(
                        ...
                        argv=[
                            "-n",
                            "needle",
                            "roboz_solutions/game_player/game_player_data/conversations/",
                        ],
                    )
                ],
            ),
            [
                [
                    ...
                    "-n",
                    "needle",
                    "{BASE}/roboz_solutions/game_player/game_player_data/conversations",
                ]
            ],
        ),
        (
        ...
        (
            RunFileCommands(
                ...
                file_commands=[
                    RunFileCommand(
                        command="find", argv=[".", "-type", "d", "-name", "roboz"]
                    ),
                    RunFileCommand(
                        command="find",
...
def test_negative_denies_relative_path_outside_allowed_scope(
    ...
    guard_payloads = _caller_payloads(messages, "guard_operation")
