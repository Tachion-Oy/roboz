...
from roboz.llm import MockLLMEndpoint, get_truncated_messages_for_context
from roboz.models import LIGHT_MAX_CHARS, Message, Role, Severity, Stop, Truncation
...
from roboz.standard.skills.cli_commands.tools import get_run_file_command
from roboz.standard.skills.cli_commands.tools.specs import (
    FILE_COMMANDS_READ,
    FILE_COMMANDS_WRITE,
)
...
def test_cp_recursive_copies_directory(tmp_path: Path) -> None:
    ...
    input_cmd = RunFileCommands(
        chain="pipe",
        file_commands=[
            RunFileCommand(command="cp", argv=["-r", "source", "copied"])
        ],
    )
    _, messages = _invoke_cli_with_tools(tools, input_cmd)
    result = _last_execute_file_command_value(messages)
...
def test_cp_overwrite_requires_delete_on_destination(tmp_path: Path) -> None:
    ...
    assert callers.count("guard_operation") >= 1
...
def test_and_touch_then_tee_allows_when_delete_unmatched_and_default_is_allow(
    ...
    assert callers.count("guard_operation") >= 2
...
def test_and_touch_then_tee_runs_through_agent_routing(tmp_path: Path) -> None:
    ...
    # Command 1 path: run_file_command -> guard_operation -> execute_file_command
    # Command 2 path: continue_file_commands -> guard_operation -> execute_file_command
    assert "continue_file_commands" in callers
    assert callers.count("guard_operation") >= 2
    assert callers.count("execute_file_command") >= 2
...
def test_pipe_three_commands_run_full_guard_cycle(tmp_path: Path) -> None:
    ...
    assert callers.count("continue_file_commands") == 2
    assert callers.count("guard_operation") == 3
...
def test_and_denied_later_command_breaks_before_execution(tmp_path: Path) -> None:
    ...
    denied_entries = _caller_entries(messages, caller="guard_operation")
