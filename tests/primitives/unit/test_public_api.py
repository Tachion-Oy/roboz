...
def test_golden_path_identities() -> None:
    assert roboz.Agent is CoreAgent
    assert roboz.tool is tooling_tool
    assert roboz.factory is tooling_factory
    assert roboz.stop is tools_stop
    assert roboz.prompt_user is tools_prompt_user
    assert roboz.prompt_agent is agent_prompt_agent
...
def test_domain_imports_do_not_replace_top_level_tool() -> None:
    ...
    assert roboz.tool is original is tooling_tool
...
def test_all_exported_ctx_contracts_inherit_factory_ctx() -> None:
    for module in (roboz, roboz.agent, roboz.tools, roboz.tooling):
        for name in module.__all__:
            value = getattr(module, name)
            if name.endswith("Ctx"):
                assert inspect.isclass(value)
                assert issubclass(value, roboz.FactoryCtx)
