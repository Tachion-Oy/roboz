...
def test_spec_serialization_keeps_public_module_identity(spec):
    assert type(spec).__module__ == "roboz.endpoints.inventory"
...
def test_discovery_does_not_import_sdk_or_read_credentials():
    ...
    subprocess.run(
        [
            ...
            from roboz.endpoints.inventory import openrouter, cerebras, groq
...
def test_collections_share_one_catalogue_class():
    import roboz.endpoints as endpoints
    from roboz.endpoints import catalog, inventory
    ...
    for name, collection in inventory.CATALOGS.items():
        ...
        assert not hasattr(endpoints, name)
        assert not hasattr(catalog, name)
    assert catalog.__all__ == ["Catalog"]
    with pytest.raises(AttributeError):
