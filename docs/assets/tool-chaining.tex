...
\begin{tikzpicture}[font=\sffamily]
  ...
  \node[anchor=west,font=\sffamily\large,text=muted]
    at (-1.45,1.05) {Every tool output goes back to the agent.};
  ...
  \node[tool] (loop-tool-1) at (7,0) {
    ...
    {\large\bfseries Ask Bob for lunch}
  };
  \node[agent] (loop-agent-2) at (0.5,-3.3) {
    {\scriptsize\bfseries\color{purple}AGENT}\\[-0.2mm]
    {\large\bfseries Search}
  };
  \node[tool] (loop-tool-2) at (7,-3.3) {
    {\scriptsize\bfseries\color{blue}TOOL}\\[-0.2mm]
    {\large\bfseries Find place}
  };
  \node[agent] (loop-agent-3) at (0.5,-6.6) {
    {\scriptsize\bfseries\color{purple}AGENT}\\[-0.2mm]
    {\large\bfseries Booking}
  };
  \node[tool] (loop-tool-3) at (7,-6.6) {
    {\scriptsize\bfseries\color{blue}TOOL}\\[-0.2mm]
    {\large\bfseries Book table}
  };
  \node[agent] (loop-resume) at (0.5,-10) {
    {\scriptsize\bfseries\color{purple}AGENT}\\[-0.2mm]
    {\large\bfseries \{ repeat until done \}}
  };
  ...
  \draw[flow] (loop-tool-3.south) -- ++(0,-0.38) -| (loop-resume.north);

  \node[anchor=west,font=\sffamily\bfseries\LARGE,text=ink]
    at (11.05,1.55) {A RoboZ tool chain};
  \node[anchor=west,font=\sffamily\large,text=muted]
    at (11.05,1.05) {Deterministic after triggering};
