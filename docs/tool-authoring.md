...
from roboz import Empty, FactoryCtx, Message, Str, factory

@dataclass(frozen=True)
class PrefixCtx(FactoryCtx):
    prefix: str

@factory
def summarize_with_prefix(
    input: Empty, messages: list[Message], ctx: PrefixCtx
) -> Str:
    return Str(value=f"{ctx.prefix} done")
...
from roboz import ExecutableDependency, FactoryCtx, ToolDependency

@dataclass(frozen=True)
class ConvertCtx(FactoryCtx):
    converter: ToolDependency[ExecutableDependency]

@factory
def convert(input: Str, messages: list[Message], ctx: ConvertCtx) -> Str:
...
from roboz import FactoryCtx, Message, Str, factory
...
@dataclass(frozen=True)
class SummarizeCtx(FactoryCtx):
    endpoint: EndpointBinding

@factory
def summarize_with_llm(
    input: Str, messages: list[Message], ctx: SummarizeCtx
) -> Str:
...
Within a standard-library skill package, the public module that constructs or exposes tools
