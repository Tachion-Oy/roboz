...
- **Invoked by an agent**: The  `prompt_agent` tool is responsible for asking an llm what to do next and the output `Invoke` type is always calling another tool. This is constructed internally in the agent on init (by setting `is_agentic), but it still is just a tool.
...
RoboZ then collapses to the traditional agentic approach as a special case if one just has the `prompt_agent` as the default with no chaining. It is possible and often useful to have an non-agentic process (`is_agentic=false` => no `prompt_agent`) where the default tools simply perform tasks (and don't consult an agent), giving rise to workflows with deterministic branching via chaining. This is useful for example when creating a *background agent* that is performing periodic maintenance work.
