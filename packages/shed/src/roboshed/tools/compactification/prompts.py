"""Instructions for compacting an active conversation into a continuation handoff."""
...
4. Preserve user intent, constraints, preferences, decisions, unresolved
   questions, blockers, and next actions when relevant to the selected skill.
...
7. Exclude greetings, filler, repeated tool output, transient details,
   superseded decisions, and information the selected skill says to ignore.
...
instructions are present, follow them over generic workflow and over the
task-specific skill.
...
COMPACTIFICATION_CONTINUATION_SKILL_MESSAGE: Final[
    str
] = """This compactification mode preserves the state
