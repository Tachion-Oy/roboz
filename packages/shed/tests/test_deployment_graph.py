...
def test_child_slot_selects_invocation_behavior(background, tmp_path):
...
def test_deployment_collects_nested_backgrounds_and_isolates_sinks_and_state(tmp_path):
    ...
    for name, pipe in pipes.items():
        assert pipe.data_path == deployment.sandbox.project_logs_dir() / name
...
def test_deployment_rejects_cross_branch_names_before_build(collision, tmp_path):
