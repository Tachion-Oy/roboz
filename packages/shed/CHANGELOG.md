...
- Breaking: email factories now use the central `EmailContext`; attachment
  ...
  See the [context migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/shed-tool-contexts.md#email-services-and-contexts).
...
- Breaking: bind built-in file and maintenance factories to concrete typed contexts
  ...
  context creates fresh state. See the [tool-context migration](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/shed-tool-contexts.md).
...
- Breaking: replace the setter-based `RoboSprawl` class with `robosprawl(sandbox, ...)`.
  ...
  agents. See [the recipe guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).
...
- Breaking: remove `Deployment` and replace the callable `RoboSprawl` dataclass
  ...
  Use one configuration instance per new run; see [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).
...
See [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md) for migration.
...
- Move `ModelSelector` from `roboshed.deployments.models` to
  ...
  behavior is unchanged. See [the archived context migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/context-migration.md).
...
- Move registration and binding from `roboshed.dependencies.contract` to
  ...
  dependency package is removed. See [the archived context migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/context-migration.md).
...
- Breaking: move email input models into `roboshed.tools.email.inputs`; they
  ...
  from `AgentDefinition` and Shed capabilities; see [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).
...
- Breaking: compose the Librarian through its `capabilities` argument, like the
  ...
  the aggregate tuning class is removed. See [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).
...
- Breaking: capabilities pass configured endpoints directly to their tools,
  ...
  See [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).

- Add `roboshed.deployments.robosprawl.AgenticFactory` to bind project persistence, initial memory, and background-start capabilities to configured definitions. Builds return `RoboSprawlBundle(agent, background_agents)`, a named tuple for direct invocation and host cancellation; construction starts no threads. See [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).

- Breaking: own orchestrator/Librarian presets, workspace/project structure, reusable capabilities, and memory/summarization tools. Import presets from `roboshed.agents`, feature implementations from `roboshed.capabilities`, and explicit paths/permission inputs from `roboshed.workspace`. Both presets return `AgentDefinition`; the orchestrator remains available across tasks until asked to stop. Preset extensions use only `capabilities`; move separate tool/skill inputs into capability builds. See [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md).
...
- **Breaking:** Replace specialized tool contexts with `roboz.Ctx(**values)`
  ...
  defaults and guard behavior. See [the archived context migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/context-migration.md) in the repository
...
- Reject relative project persistence paths that escape through traversal or
  ...
  see [the archived agent migration guide](https://github.com/Tachion-Oy/roboz/blob/roboshed-v0.1.1.dev4/docs/agent-factories.md) for migration guidance.
