...
def fetch_reply_source(
    ...
    *,
    ...
    with sessions.open() as connection:
        ...
        if not recipients:
            raise EmailProviderError("email reply source has no valid reply address")
        if len(recipients) + len(cc) > MAX_REPLY_RECIPIENTS:
            raise EmailProviderError("email reply source has too many reply addresses")
        ...
        if include_quoted_original:
            ensure_not_cancelled(is_cancelled)
            quoted_body, quote_warnings = _fetch_quoted_body(connection, reference.uid)
        return ReplySourceHeaders(
            ...
            sent_at=display_header(headers, EmailHeader.DATE, default="", max_chars=500)
            or None,
...
def _fetch_quoted_body(
    ...
    warnings = (
        ("The quoted original message was truncated.",) if extracted.truncated else ()
    )
