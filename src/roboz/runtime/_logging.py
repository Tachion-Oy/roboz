...
# Format used by `reset_logging_config` only. Agent never configures the root logger
# internally—doing so from library code would affect every importer.
#
# Some third-party LLM SDKs still mutate ``logging`` at import time. Agent pulls those
...
# The thread name and millisecond timestamp are deliberate: Agent runs external
...
def reset_logging_config(level: int = logging.INFO) -> None:
    """Opt-in helper: replace root handlers with Agent's stderr formatter.

    **Agent must not call this from its own library code.** It clears
