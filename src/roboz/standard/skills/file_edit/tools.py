"""Single-file apply-patch tool for the file-edit skill."""

from dataclasses import dataclass
from pathlib import Path

from roboz import FactoryCtx, factory
from roboz.models import Message, Severity, Str, Truncation, TruncationSpec
from roboz.runtime import EventPipe
from roboz.tooling import Tool

from roboz.standard.identifiers import APPLY_PATCH_TOOL_NAME, FILE_EDIT_SKILL_NAME
...
def get_apply_patch(
    *,
    ...
    file_edit_skill_name: str = FILE_EDIT_SKILL_NAME,
) -> list[Tool]:
    """Single-file apply_patch: same guard chain as CLI tools, Python replace executor.

    Args:
        file_edit_skill_name: Skill id (``Skill.name``) referenced in the tool
            description as the source of detailed file-edit usage instructions.
            Defaults to ``FILE_EDIT_SKILL_NAME``.
    """
    ...
    description = (
        ...
        f"The possibly available `{file_edit_skill_name}` skill provides detailed "
