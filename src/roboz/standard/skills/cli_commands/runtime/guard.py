...
from .guard_formatting import format_guard_constraints
from .types import GuardCtx, ResolvedFileCommand
from .utils import check_allow_deny_permission, check_ask_permission
...
def guard_items(
    *,
    items_to_guard: list[GuardFileSingle],
...
def build_guarded_tool_chain(
    *,
    ...
    Inserts the shared ``guard_operation`` between an already named/described
    ...
    """
    guard = guard_operation(guard_ctx).copy(
        chained_to=entry,
