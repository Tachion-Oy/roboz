...
from ..contracts import EmailMailbox
...
def store_draft(
    ...
    *,
    ...
    """Append a draft after checking for an existing request ID."""
    ensure_not_cancelled(is_cancelled)
    with sessions.open() as connection:
        drafts_mailbox = mailbox_for(connection, EmailMailbox.DRAFTS)
        ensure_not_cancelled(is_cancelled)
        if request_id:
            existing = _find_by_request_id(
                connection, drafts_mailbox, request_id
            )
...
def probe_mailboxes(sessions: ImapSessionProvider) -> dict[str, object]:
    """Authenticate and report mailbox names without changing messages."""
...
def _find_by_request_id(
    connection: Any, mailbox: str, request_id: str
) -> str | None:
