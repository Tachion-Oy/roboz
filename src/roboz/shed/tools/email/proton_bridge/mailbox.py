...
def mailbox_for(connection: Any, mailbox: EmailMailbox) -> str:
    """Resolve a logical mailbox through Bridge special-use flags."""
...
def select_mailbox(connection: Any, mailbox: str, *, readonly: bool) -> int:
    """Select a mailbox and return its current UIDVALIDITY."""
...
def source_reference(
    ...
    """Verify a source reference belongs to the requested mailbox."""
    ...
    if reference.mailbox.casefold() != expected_mailbox.casefold():
        raise EmailProviderError("email source does not belong to the requested mailbox")
    return reference


def require_current_reference(actual: int, expected: int) -> None:
    """Reject references made before mailbox UIDVALIDITY changed."""
    if actual != expected:
...
def fetch_headers(connection: Any, uid: bytes) -> tuple[Message, datetime | None]:
    """Fetch bounded headers and reception metadata for one UID."""
...
def fetch_full_message(connection: Any, uid: bytes) -> tuple[bytes, bytes]:
    """Fetch one message with a strict size limit."""
