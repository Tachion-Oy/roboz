...
from ..contracts import EmailProviderError, EmailSearchRequest
...
def mailbox_name(list_response: str) -> str:
    """Extract the mailbox name from an IMAP LIST response."""
...
def append_uid(response: list[bytes]) -> str | None:
    """Extract the appended message UID when the server supplies it."""
...
def search_criteria(request: EmailSearchRequest) -> tuple[str | bytes, ...]:
    """Translate structured search filters into IMAP arguments."""
    ...
    if request.since:
        criteria.extend(
            (ImapSearchKey.SINCE, request.since.strftime("%d-%b-%Y"))
        )
    if request.before:
        criteria.extend(
            (ImapSearchKey.BEFORE, request.before.strftime("%d-%b-%Y"))
        )
    return tuple(criteria or (ImapSearchKey.ALL,))


def imap_quote(value: str) -> bytes:
    """Quote a search term while rejecting control characters."""
...
def uid_validity(response: object) -> int:
    """Parse UIDVALIDITY from a selected mailbox response."""
...
def response_value(response: object) -> bytes:
    """Get the bytes carried by an IMAP response tuple."""
...
def fetch_response_parts(response: object) -> tuple[bytes, bytes | None]:
    """Separate IMAP metadata from fetched message bytes."""
...
def internal_date(metadata: bytes) -> datetime | None:
    """Parse the server reception date from fetch metadata."""
...
def uid_message_metadata(
    ...
    for item in response:
        ...
        if uid not in seen_uids:
            parsed.append((uid, received_at, br"\seen".lower() in flags))
            seen_uids.add(uid)
    return tuple(parsed)
